...
const Home = ({ isOpen }) => {
  const [activeName, setActiveName] = useState("Search for help");
  const images = [
  ...
  const titleParagraphs = [
    {
      title: "Hiring on Dribbble",
      paragraph:
        "Send requests, review proposals, and pay securely in one place.",
    },
    {
      title: "Why choose Dribble for your project?",
      paragraph: "Get access to top design talent and payment protection.",
    },
  ];

  const titleIcone = [
    {
      name: "Submit an issure",
      icone: <Wrench size={17} />,
    },
    {
      name: "Account Suspension",
      icone: <BookUser size={17} />,
    },
    {
      name: "Refund Request",
    ...
    },
    {
      ...
      icone: <Building size={17} />,
    },
  ];

  const namesIcone = [
    { name: "Search for help", icone: <Search size={17} strokeWidth={3.2} /> },
    {
      name: "How do I get paid on Dribbble?",
      icone: <ChevronRight size={17} strokeWidth={3.2} />,
    },
    {
      name: "Getting Hired on Dribbble",
      icone: <ChevronRight size={17} strokeWidth={3.2} />,
    },
    {
      name: "What are Dribbble Projects?",
      icone: <ChevronRight size={17} strokeWidth={3.2} />,
    },
    {
      name: "Hiring on Dribbble",
      icone: <ChevronRight size={17} strokeWidth={3.2} />,
    },
  ];

  return (
    <div className="custom-scrollbar relative h-[86%] w-full overflow-auto overscroll-contain bg-neutral-100/20">
      <div className="absolute w-full rounded-4xl">
        <div
          onClick={() => isOpen(false)}
          className="fixed top-8.5 right-5 z-1000 rounded-lg p-2 transition-all duration-200 hover:cursor-pointer hover:bg-neutral-200 min-[450px]:top-14 min-[450px]:right-13"
        >
          <X size={18} />
        </div>
        ...
        <div className="mt-20 flex items-center gap-20 pl-10 min-[450px]:mt-10">
          ...
          <div className="relative flex h-10 w-24 items-center">
            {images.map((image, index) => (
              <img
                key={index}
                className="absolute size-12 rounded-full border-4 border-white object-cover"
                src={image}
                style={{
                  right: `${index * 40}px`,
                }}
        ...
        <div className="flex flex-col items-center gap-3">
          <div className="mx-10 mt-3 flex w-77 flex-col justify-between gap-1 rounded-xl border-2 border-neutral-300 bg-white px-3 py-4 shadow-sm min-[450px]:w-85">
            {namesIcone.map((name) => (
              <div
                ...
                className={`${activeName.toLowerCase() === name.name.toLowerCase() ? "bg-neutral-200/60 font-medium text-black" : "bg-white text-neutral-600"} flex items-center justify-between rounded-lg px-3 py-3 transition-all duration-300 hover:cursor-pointer hover:bg-neutral-200`}
              >
                <p className="text-sm">{name.name}</p>
                <span className="">{name.icone}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex w-77 flex-col justify-between gap-4 rounded-xl border border-neutral-300 bg-white px-5 py-4 shadow-sm min-[450px]:w-85">
            ...
            {titleIcone.map((name) => (
              <div
                className={`flex items-center justify-between rounded-lg bg-white font-normal text-neutral-900 transition-all duration-300 hover:cursor-pointer`}
              >
                <p className="text-sm">{name.name}</p>
                <span className="">{name.icone}</span>
              </div>
          ...
          <div className="mb-2 flex w-77 flex-col justify-between gap-0 rounded-xl border border-neutral-300 bg-white pb-3 shadow-sm hover:cursor-pointer min-[450px]:w-85">
            <div className="h-40 w-full rounded-xl object-center">
              <img
                ...
                src="https://downloads.intercomcdn.com/i/o/ikho1pf5/1892911866/17cfaf8a9fd1f8a9807187345f60/Screenshot+2025-12-18+at+2_13_55%E2%80%AFPM.png?expires=1768039200&signature=f75bb158cfb039a30cb489b98088ecb56755b253af1627d78f096c27108622a4&req=dSguFMB%2FnIlZX%2FMW1HO4zbiXCwoudhd9ZwKg7AK6XcDPa4%2FXEQmrztyXMBNV%0A7u8lBeY%3D%0A"
                alt=""
              />
            </div>
            <div className="mt-5 px-5">
              <p className="w-28 rounded-full bg-neutral-200 p-1 text-center text-xs font-medium">
                Feature update
              </p>
              <div className="mt-4 flex items-center justify-between gap-5">
                <div className="flex flex-col gap-2">
                  <h1 className="text-xs font-semibold">
                    Designers can now generate more leads than ever with
                    Dribbble Pro.
                  </h1>
                  <p className="line-clamp-2 h-auto max-w-70 text-xs leading-5 font-normal">
                    Designers now have multiple Pro plans to choose from,
                    depending on the level of control they want over their
                    earnings on Dribbble:
                  </p>
                </div>
