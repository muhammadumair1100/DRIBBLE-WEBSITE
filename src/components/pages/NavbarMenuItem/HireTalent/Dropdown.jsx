...
const Dropdown = () => {
  ...
  const bannerIcones = [
    { icone: <House strokeWidth={1.6} />, name: "Home" },
    { icone: <MessageSquareText strokeWidth={1.6} />, name: "Messages" },
    { icone: <CircleQuestionMark strokeWidth={1.6} />, name: "Help" },
    { icone: <Megaphone strokeWidth={1.6} />, name: "News" },
  ];
    ...
    <div className="fixed right-7 bottom-10 z-100">
      <div className="relative">
        <button
          ...
          className="relative rounded-full bg-pink-300 p-2.5 hover:cursor-pointer"
        >
          <svg
        ...
        {isOpen && (
          ...
          >
            {activeItem === "Home" ? (
              ...
              <Help isOpen={setIsOpen} />
            ) : (
              <News isOpen={setIsOpen} />
            )}
            <div className="absolute right-0 bottom-0 z-100 flex w-full items-center justify-center gap-12 border border-neutral-300 bg-white py-4 min-[450px]:rounded-b-3xl">
              {bannerIcones.map((icone) => (
                <div
                  onClick={() => setActiveItem(icone.name)}
                  className="group z-100 flex flex-col items-center gap-1 hover:cursor-pointer hover:text-black"
                >
                  <span
                    className={`${activeItem.toLowerCase() === icone.name.toLowerCase() ? "text-black" : "text-neutral-500 group-hover:text-black/80"}`}
                  >
                    {icone.icone}
                  </span>
                  <span
                    className={` ${activeItem.toLowerCase() === icone.name.toLowerCase() ? "font-semibold text-black" : "font-medium text-neutral-500 group-hover:text-black/80"} text-sm`}
                  >
                    {icone.name}
                  </span>
                </div>
